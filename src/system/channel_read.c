...
BOOL channel_read_from_ir(Channel*pch,READ_TYPE_CMD cmd,U8*dataout,int ms_timeout)
{
	  U8 wanlen = 0;
	  int result = 0;
	  U16 buffer[8]= {0,0,0,0,0,0,0,0};//缓冲数据
		switch(cmd)
		{
	  ...
	  ld_ir_read_start(pch->map->io_ir,pch->map->io_re,cmd,wanlen);  //启动命令
		while((result=ld_ir_read_isok((U8*)(buffer),wanlen))==1)//等待结束,超时时间ms_timeout ms
		{
			delayms(10);
			ms_timeout-=10;
			if(ms_timeout<0){
	  ...
	  result = channel_read_from_iic(pch,cmd,dataout);
		if(result==FALSE){
			...
			if(!force_using_ir())//非强制使用iic
			{
				...
				{
					pch->iic_error_counter=BAO_DINGZHEN_ERROR_TIMES;
					delayms(50);
